...
const Dashboard = () => {
  ...
  sales.forEach((sale: any) => {
    const monthKey = new Date(sale.sale_date || sale.created_at).toLocaleString("default", {
      month: "short",
      year: "numeric",
    });
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
        <MetricCard label="Total Revenue" value={`$${formatNumber(totalRevenue.toFixed(2))}`} color="green" />
        <MetricCard label="Units Sold" value={formatNumber(totalUnits)} color="blue" />
        <MetricCard label="Categories Tracked" value={formatNumber(Object.keys(totalsByCategory).length)} color="purple" />
      </div>
